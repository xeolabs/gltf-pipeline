...
var process = inputIsBinary ? (outputIsBinary ? processGlb : glbToGltf) : (outputIsBinary ? gltfToGlb : processGltf);

console.time('Total');

read(inputPath)
    .then(function(gltf) {
        return process(gltf, options);
    })
    .then(function(gltf) {
        return write(outputPath, gltf, writeOptions);
    })
    .then(function() {
